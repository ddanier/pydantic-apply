...
test *args: (uv "run" "pytest" "--cov=pydantic_apply" "--cov-report" "term-missing:skip-covered" args)

test-all: (uv "run" "tox")

ruff *args: (uv "run" "ruff" "check" "pydantic_apply" "tests" args)

pyright *args: (uv "run" "pyright" "pydantic_apply" args)
